...
export default function OrbitMap() {
  ...
  useEffect(() => {
    ...
    for (const n of names) {
      ...
      meta[n] = { color: p.color || "#8899AA", ring: !!p.ring, size: Math.max(3, Math.min(9, 3 + (p.size || 1) * 0.55)) };
    }
    ...
    function resize() {
      ...
      h = Math.max(380, Math.min(560, Math.round(w * 0.52)));
      canvas.width = w * dpr; canvas.height = h * dpr;
      canvas.style.height = `${h}px`;
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      cx = w / 2; cy = h / 2 + 10;
      scale = Math.min(w / 2.15, (h / 2.1) / TILT) / RMAX;
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-hairline px-4 py-3">
        <div className="flex flex-wrap items-center gap-1.5">
            ...
            <button
              ...
              className={`rounded border px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider transition ${
                warpIdx === i
                  ...
                  : "border-hairline text-dim hover:border-telemetry/30 hover:text-starlight"
              }`}
            >
              {wp.label}
            </button>
          ...
          <button
            ...
            className="rounded border border-hairline px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider text-dim transition hover:border-signal/40 hover:text-signal"
          >
            ↺ Today
          </button>
        </div>
